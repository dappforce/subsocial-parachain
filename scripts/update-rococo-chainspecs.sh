...
docker exec -it -u root rococo-export sh -c "polkadot build-spec --chain rococo-local --disable-default-bootnode > /rococo-local"
docker exec -it -u root rococo-export sh -c "polkadot build-spec --chain rococo > /rococo"
docker exec -it -u root rococo-export sh -c "polkadot build-spec --chain /rococo-local --raw --disable-default-bootnode > /rococo-local.json"
