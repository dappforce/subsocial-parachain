...
use mock::{Balance, MaxUnlockingChunks};
use sp_runtime::traits::Zero;
use crate::tests::mock::MaxEraStakeItems;

#[test]
fn unbonding_info_test() {
    let mut unbonding_info = UnbondingInfo::<Balance, MaxUnlockingChunks>::default();

    // assert basic ops on empty info
    assert!(unbonding_info.is_empty());
    assert!(unbonding_info.len().is_zero());
    let unlocking_chunks = unbonding_info.vec();
    let (first_info, second_info) = UnbondingInfo { unlocking_chunks }.partition(2);
    ...
    let unlocking_chunks = unbonding_info.vec();
    let (first_info, second_info) = UnbondingInfo { unlocking_chunks }.partition(base_unlock_era);
    ...
    let unlocking_chunks = unbonding_info.vec();
    let (first_info, second_info) = UnbondingInfo { unlocking_chunks }.partition(partition_era);
...
fn stakes_info_basic() {
    let backer_info = StakesInfo::<Balance, MaxEraStakeItems>::default();

    assert!(backer_info.is_empty());
    assert_eq!(backer_info.len(), 0);
    assert_eq!(backer_info.current_stake(), 0);
}

#[test]
fn stakes_info_stake_ops() {
    let mut backer_info = StakesInfo::<Balance, MaxEraStakeItems>::default();

    // Do first stake and verify it
    let first_era = 1;
    let first_stake = 100;
    assert_ok!(backer_info.increase_stake(first_era, first_stake));
    assert!(!backer_info.is_empty());
    assert_eq!(backer_info.len(), 1);
    assert_eq!(backer_info.current_stake(), first_stake);

    // Do second stake and verify it
    let second_era = first_era + 1;
    let second_stake = 200;
    assert_ok!(backer_info.increase_stake(second_era, second_stake));
    assert_eq!(backer_info.len(), 2);
    assert_eq!(
        backer_info.current_stake(),
    ...
    let third_stake = 333;
    assert_ok!(backer_info.increase_stake(third_era, third_stake));
    assert_eq!(
        backer_info.current_stake(),
    ...
    assert_ok!(backer_info.increase_stake(fourth_era, fourth_stake));
    assert_eq!(backer_info.len(), 3);
    assert_eq!(
        backer_info.current_stake(),
...
fn stakes_info_stake_error() {
    let mut backer_info = StakesInfo::<Balance, MaxUnlockingChunks>::default();
    assert_ok!(backer_info.increase_stake(5, 100));
    if let Err(_) = backer_info.increase_stake(4, 100) {
    } else {
...
fn stakes_info_unstake_ops() {
    ...
    // Unstake on empty backer_info
    assert!(backer_info.is_empty());
    assert_ok!(backer_info.unstake(1, 100));
    assert!(backer_info.is_empty());
    ...
    assert_ok!(backer_info.unstake(first_unstake_era, first_unstake));
    assert_eq!(backer_info.len(), 2);
    assert_eq!(
        backer_info.current_stake(),
    ...
    assert_ok!(backer_info.unstake(second_unstake_era, second_unstake));
    assert_eq!(backer_info.len(), 3);
    assert_eq!(
        backer_info.current_stake(),
    ...
    // Save this for later
    let stakes = backer_info.stakes.clone();
    let temp_backer_info = StakesInfo { stakes };

    // Fully unstake existing EraStake
    assert_ok!(backer_info.unstake(second_unstake_era, total_staked));
    assert_eq!(backer_info.len(), 3);
    assert_eq!(backer_info.current_stake(), 0);

    // Fully unstake non-existing EraStake
    let mut backer_info = temp_backer_info; // restore
    assert_ok!(backer_info.unstake(second_unstake_era + 1, total_staked));
    assert_eq!(backer_info.len(), 4);
    assert_eq!(backer_info.current_stake(), 0);
}
...
fn stake_after_full_unstake() {
    ...
    assert_ok!(backer_info.unstake(unstake_era, first_stake));
    assert!(backer_info.current_stake().is_zero());
    assert_eq!(backer_info.len(), 2);

    // Stake again in the next era
    let restake_era = unstake_era + 2;
    let restake_value = 57;
    assert_ok!(backer_info.increase_stake(restake_era, restake_value));
    assert_eq!(backer_info.current_stake(), restake_value);
    assert_eq!(backer_info.len(), 3);
}
...
fn stakes_info_unstake_error() {
    let mut backer_info = StakesInfo::<Balance, MaxUnlockingChunks>::default();
    assert_ok!(backer_info.increase_stake(5, 100));
    if let Err(_) = backer_info.unstake(4, 100) {
    } else {
...
fn stakes_info_claim_ops_basic() {
    ...
    // Empty backer info
    assert!(backer_info.is_empty());
    assert_eq!(backer_info.claim(), (0, 0));
    assert!(backer_info.is_empty());

    // Only one unstaked exists
    assert_ok!(backer_info.increase_stake(1, 100));
    assert_ok!(backer_info.unstake(1, 100));
    assert!(backer_info.is_empty());
    assert_eq!(backer_info.claim(), (0, 0));
    assert!(backer_info.is_empty());
    ...
    backer_info = StakesInfo::<Balance, MaxUnlockingChunks>::default();
    let stake_era = 1;
    let stake_value = 123;
    assert_ok!(backer_info.increase_stake(stake_era, stake_value));
    assert_eq!(backer_info.len(), 1);
    assert_eq!(backer_info.claim(), (stake_era, stake_value));
    assert_eq!(backer_info.len(), 1);
}
...
fn stakes_info_claim_ops_advanced() {
    ...
    assert_ok!(backer_info.increase_stake(first_stake_era, first_stake_value));
    assert_ok!(backer_info.increase_stake(second_stake_era, second_stake_value));
    assert_ok!(backer_info.increase_stake(third_stake_era, third_stake_value));

    // First claim
    assert_eq!(backer_info.len(), 3);
    assert_eq!(backer_info.claim(), (first_stake_era, first_stake_value));
    assert_eq!(backer_info.len(), 2);
    ...
    assert_ok!(backer_info.unstake(5, total_staked));
    assert_eq!(backer_info.len(), 2);
    ...
    // Claim 4th era
    assert_eq!(backer_info.claim(), (third_stake_era, total_staked));
    assert_eq!(backer_info.len(), 1);
