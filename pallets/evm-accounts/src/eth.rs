...
/// Eth Address.
pub(crate) type EthAddress = H160;
...
impl<T: Config> Pallet<T> {
    ...
    pub(crate) fn verify_eip712_signature(
        ...
        recover_eth_signer(sig, &message.message_hash())
    }
}

fn recover_eth_signer(sig: &Eip712Signature, msg_hash: &MessageHash) -> Option<EthAddress> {
...
impl<T: Config> SingableMessage<T> {
    ...
    pub(crate) fn payload_hash(&self) -> MessageHash {
        match self {
            ...
            SingableMessage::EthAddressCall { call_hash, account_nonce} => {
                let tx_type_hash = keccak256!("Transaction(string transactionName,bytes callHash, uint256 accountNonce)");
                let mut tx_msg = tx_type_hash.to_vec();
                tx_msg.extend_from_slice(keccak256!("EthAddressCall")); // transactionName
