...
use sp_runtime::{
    traits::{
        Bounded, CheckedAdd, CheckedSub, DispatchInfoOf, Extrinsic, Hash, MaybeSerialize,
...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        /// Account have been mapped to ethereum address
        AccountMapped {
    ...
    #[pallet::error]
    pub enum Error<T> {
        /// The ethereum address have an existing mapping already.
        EthAddressAlreadyMapped,
        /// The substrate address have an existing mapping already.
        AccountAlreadyMapped,
        /// The provided signature is invalid
        BadSignature,
        /// There is no mapped substrate address to the given ethereum address
        NoMappedAccount,
    }
    ...
    pub type Accounts<T: Config> =
        StorageMap<_, Twox64Concat, EthAddress, T::AccountId, OptionQuery>;
    ...
    impl<T: Config> Pallet<T> {
        /// Map Ethereum address to substrate address.
        #[pallet::call_index(0)]
        #[pallet::weight(Weight::from_ref_time(340_000_000)
            .saturating_add(T::DbWeight::get().reads(3 as u64))
            .saturating_add(T::DbWeight::get().writes(2 as u64)))]
        pub fn map_eth_account(
            origin: OriginFor<T>,
            ...
            ensure!(!EthAddresses::<T>::contains_key(&who), Error::<T>::AccountAlreadyMapped);
            ensure!(!Accounts::<T>::contains_key(eth_address), Error::<T>::EthAddressAlreadyMapped);

            let message = SingableMessage::MapEthAddress {
            ...
            Self::deposit_event(Event::AccountMapped { substrate: who, ethereum: eth_address });
        ...
        <T as frame_system::Config>::RuntimeCall: IsType<<T as Config>::RuntimeCall>,
    {
        ...
        fn validate_unsigned(
                ...
                call.is_sub_type()
            {
                ...
                let extra: ChargeTransactionPayment<T> = ChargeTransactionPayment::<T>::from(tip);
                let call = <T as frame_system::Config>::RuntimeCall::from_ref(call);
                return extra.validate(&mapped_account, call, info, len)
            }
